...
urlpatterns = [
    ...
    path("<int:pk>/tab_reorder/", views.tab_reorder, name="tab_reorder"),#タブ順番入れ替え
    path('plan_edit/<int:pk>/', views.plan_edit, name='plan_edit'),
    path('plan_delete/<int:pk>/', views.plan_delete, name='plan_delete'),
    path('share/<str:token>/', views.plan_share, name='plan_share'),
    path('share/revoke/<int:member_id>/', views.share_revoke, name='share_revoke'),
    ...
    path('schedule/<int:pk>/remove/', views.schedule_remove, name='schedule_remove'), #目的地を1日詳細からはずす
